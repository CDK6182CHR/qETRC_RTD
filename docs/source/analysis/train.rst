...
.. figure:: /_static/img/analysis/timetable-diagon.png
    :alt: 时刻诊断-V1.7.2
...
:起始版本:
:pyETRC对应功能: 运行图对比
:工具栏: :guilabel:`列车(3)` | :guilabel:`运行图对比`

此功能提供当前 **运行图文件** 与另一运行图文件中的 **所有车次** 的对比。

.. figure:: /_static/img/analysis/diagram-compare.png
    :alt: 运行图对比-V1.7.3

如上图所示为一简单示例。点击 :guilabel:`浏览` 选择运行图文件后，下方表格中将显示当前运行图与所选运行图文件间的车次差异。
表中每一行是一个车次的对比。以当前运行图为准，从当前运行图到所选运行图，列车可能有下述几种情况：

- 列车被删除。本运行图中存在，所选运行图中不存在的车次。在表格左侧用灰色文字显示。
- 新增列车。本运行图中不存在，所选运行图中存在的车次。在表格右侧用蓝色文字显示。
- 修改列车。两运行图中皆存在的车次，且时刻表有变更（包括车站增删、时刻调整等）。在表格中以红色文字显示。
- 列车时刻表不变。两运行图中皆存在，且时刻表完全相同的车次。以黑色文字显示。

.. note::
    列车的对比仅通过车次名确认是否为同一车次；车次名称不同的，总是不会列在同一行，即使它们的时刻表完全相同。例如，若本运行图中有一车次名A在另一运行图中车次名变为B，时刻表不变，则这里将识别为删除车次A、新增车次B。

对于新增、删除的列车，双击或通过右键菜单显示列车时刻表。对于修改或不变的列车，双击或通过右键菜单，可显示两运行图中的列车时刻表对比，如上图右侧所示。表格左侧为当前运行图的时刻表信息，右侧为同一车次在所选运行图中的时刻信息。每行车站时刻的变化包括增加、删除、时刻调整（“改点”）等情况。
其中，增加车站以蓝色显示，删除车站以灰色显示，调整的时刻以红色显示。
