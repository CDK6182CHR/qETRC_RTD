...
:起始版本: V1.0.0
:pyETRC对应功能: 无
:工具栏: :guilabel:`线路工具(8)` | :guilabel:`断面对数`
:另请参阅: :ref:`区间对数表<sec_interval_count>`

断面对数表功能提供线路上每个相邻站区间内的列车数量。断面对数可以定义为，（在显示所有运行线的情况下）在运行图上非车站位置绘制一条水平线，所相交的运行线数量，故名。显然，在运行图的模型中，同一区间内任意里程标所得的断面对数是相等的，因此此功能按区间列出数据。数据是分上下行独立给出的。
此功能与新版ETRC中的对数表功能一致。

.. figure:: /_static/img/analysis/cut-count.png
    :alt: 断面对数-V1.4.0


请注意，此功能与 :ref:`区间对数表<sec_interval_count>` 定义有所不同，体现在：

1. 断面对数表功能中，所有经过所统计区间的列车运行线都会被计入，而无论列车时刻表中是否包含该区间的两个端点站。而区间对数表中仅统计时刻表包含所求区间端点站的列车。也就是说，断面对数表功能自动推定时刻表上不包含的通过站，而区间对数表功能不做此推定。

2. 断面对数表只包含相邻车站所构成的最小区间，而区间对数表可选择/显示线路上的任意区间。

.. note::
    运行图页面右侧显示的 ``客货对数`` 栏（如果开启）与本功能逻辑一致，但分为客车和非客车（货车）独立统计。
...
:起始版本: V1.1.2
:pyETRC对应功能: 区间对数表
:工具栏: :guilabel:`列车(3)` | :guilabel:`区间对数表`
:另请参阅: :ref:`断面对数表<sec_cut_count>` , :ref:`区间车次表<sec_interval_trains>` 

区间对数表功能统计线路上 **任意两个车站** 之间的车次数，以及其中属于始发的数量、属于终到的数量、同时是始发终到的数量，同时可筛选是否仅统计停车列车或营业列车，以作为线路上车站通达度的评估。

.. figure:: /_static/img/analysis/interval-count.png
    :alt: interval-count-V1.4.0

本功能界面如上图所示。表格中所展示的是以指定车站为中心（可选发站或到站），到本线其他所有车站的区间的对数情况。可对车站和车次进行筛选。
双击表格，显示该行所对应区间的详细车次表。

.. note::
    筛选中的 ``仅停车车次`` 也包含始发终到的情况。

.. _sec_interval_trains:
...
:最新修订: V1.3.6
:pyETRC对应功能: 区间车次表
:工具栏: :guilabel:`列车(3)` | :guilabel:`区间车次表`
:另请参阅: :ref:`区间对数表<sec_interval_count>` 

此功能提供查看 **本运行图所含任意两站之间** 的车次数据，类似于购票软件中查询两站间车次。

.. figure:: /_static/img/analysis/interval-trains.png
    :alt: interval-trains-V1.4.0

表中显示为所查询区间在所设置的筛选条件下的车次及其基本信息，每行一个车次。其中， ``历时`` 列所示为该列车自发站发时起、到站到时止的时长，即不包括发站、到站的停车时长。

发站、到站可输入任意车站名（只需包含于本运行图内即可，不必在同一条线上）。

若勾选 ``使用多车站分隔符|`` ，则可以一次查询多个车站。使用分隔符 ``|`` （Unicode:  ``U+007C`` ）分隔多个可选车站，其间为逻辑“或”的关系。例如， ``成都|成都西`` 可匹配到 ``成都`` 或 ``成都西`` 。

.. warning::
    分隔符 ``|`` 两端请不要添加包括空格在内的任何其他字符。本功能不会分辨 ``|`` 符号是否属于车站站名的一部分，也不支持转义操作。因此，启用此分隔符时，请避免在车站站名内使用 ``|`` 符号。

若勾选 ``使用正则表达式`` ，则可以使用正则表达式 （regular expression）匹配多个可能的车站站名。例如， ``成都[南,西]`` 可以匹配到 ``成都南`` 和 ``成都西`` 。

.. note::
    正则表达式中域解析符 ``::`` 不起作用，整条正则表达式作为整体使用。若正则表达式和上述的多车站查询同时开启，则首先按照 ``|`` 划分，再对每个部分使用正则表达式匹配。

.. tip::
    此处匹配的判据为Qt实现的 ``QRegularExpression::match()`` 以及 ``QRegularExressionMatch::hasMatch()`` 。这里的判据默认并不要求完全匹配，例如正则表达式 ``成都`` 也可匹配到 ``成都南`` 。如果要严格匹配，可以使用行首和行尾表达，例如 ``^成都$`` 。
...
:起始版本: V1.2.0
:pyETRC对应功能: 无
:工具栏: :guilabel:`线路(8)` | :guilabel:`线路拓扑`

本功能显示线路的拓扑（topological）信息，是把线路上车站理解成类似图（graph）数据结构时的拓扑结构。具体来说，线路拓扑主要是指单向站和单线的情况。

.. figure:: /_static/img/analysis/rail-topo.png
    :alt: 线路拓扑-V1.4.0

如上图所示是线路拓扑的示例。其中，第三列（加背景色的列）为站名。左侧的里程、对里程为用户设置的原始里程标（参见 :ref:`基线编辑<sec_rail_basic>` ），右侧显示的即为线路拓扑结构信息。第四、五列所示数据为区间的里程（单位为km）。线路的特殊拓扑结构信息主要体现在单元格的合并；合并的单元格表示实际上只有一个区间。若单元格纵向合并，则表示存在单向站或者不通过的站，区间起止站为合并单元格最上、最下界限所对应的左侧站名。若单元格横向合并，则表示此区间为单线区间，上下行区间实际为同一个区间。

例如，上图中，D为上行单向站，故C-E为一个下行相邻区间；C是下行单向站，故D-B为一个上行相邻区间；F-G区间为单线区间。

.. tip::
    关于线路拓扑、单双线支持的更多定义、技术实现，可参阅专栏文章 https://zhuanlan.zhihu.com/p/584931750
