...
trf是ETRC支持的车次描述文件，它是一种文本文件，可以由ETRC软件导入或者导入。trf文件的内容实质上就是trc运行图文件中描述列车的部分，但\ **每个trf文件只能包含一个车次**\ 。qETRC提供对批量导入trf文件的支持，功能位于工具栏 :guilabel:`列车(3)` | :guilabel:`导入车次` | :guilabel:`批量导入车次(trf)` 。可以一次性选择多个trf文件，每个文件作为一个车次，导入到当前运行图文件中。以下是trf文件的一个示例。
